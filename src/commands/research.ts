...
import { generateCompanyResearch, generateQuestionsToAsk } from "../llm/research";
import type { CompanyResearchOptions } from "../llm/research";
...
export async function generateRoleResearch(
  userId: string,
  roleId: string,
...
About: ${profile.about_me || 'Not provided'}
  ...
  for await (const { section, content } of generator) {
    ...
    } else if (section === "questions_to_ask") {
      await db.unsafe(
        "UPDATE role_research SET questions_to_ask = $1, updated_at = now()::text WHERE user_id = $2 AND id = $3",
        [content, localUser.id, researchId]
      );
...
export async function generateRoleQuestionsToAsk(
  userId: string,
  roleId: string,
  guidance?: string
): Promise<string> {
...
About: ${profile.about_me || "Not provided"}
  ...
  const questionsToAsk = await generateQuestionsToAsk(
    {
      provider,
      apiKey,
      model: config.llm.model,
  ...
  await db.unsafe(
    "UPDATE role_research SET questions_to_ask = $1, updated_at = now()::text WHERE user_id = $2 AND id = $3",
    [questionsToAsk, localUser.id, researchId]
  );
